...
export default function PersonalNotes() {
    ...
    const [modalInput, modalEdit, openModal, openEditModal, closeModal] =
        useModal();
    const [filteredNotesData, filteredArchivedData, setSearchQuery] = useData(
        notesData,
        archivedData,
    );
    ...
    return (
        <>
            <div className="w-full h-[100%] min-h-[100vh] max-h-full bg-slate-900">
                ...
                />
                <Sidebar onOpen={openModal} setCurrentView={setCurrentView} />
                {modalInput && (
                    <Modal modalInput={modalInput} onClose={closeModal}>
                        <AddNote addNotes={addNote} />
                    </Modal>
                )}
                {modalEdit && (
                    <Modal modalEdit={modalEdit} onCloseEdit={closeModal}>
                        <EditNote
                            setNote={setNotes}
                            addNotes={notes}
